    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            if 'scale_factor' in self.upsample_cfg:
                laterals[i - 1] = laterals[i - 1]*self.lateral2_convs[used_backbone_levels-1-i](self.up2_convs[used_backbone_levels-1-i](laterals[i]))
            else:
                prev_shape = laterals[i - 1].shape[2:]
                laterals[i - 1] = laterals[i - 1]*self.lateral2_convs[used_backbone_levels-1-i](self.up2_convs[used_backbone_levels-1-i](laterals[i]))
        laterals =laterals + [self.down_convs[0](self.lateral3_convs[0](laterals[-1]))]
        # laterals = [
